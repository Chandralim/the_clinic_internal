<template>
    <div v-show="!ispop || (ispop && showpop)" class="w-full flex grow flex-col overflow-auto h-0 z-10" :class="ispop ?'fixed':'relative'">
        <TheHeaderPopup v-if="ispop" :title="'Form User'" :fn="fnClose" class="w-100 flex align-items-center"
        style="color:white;" />
        <div class="w-full flex grow flex-col overflow-auto h-0">
          ...
          <TheTableView :thead="fields_thead" :selected="selected" @setSelected="selected = $event" :tbody="users" :fnCallData="callData" :scrolling="scrolling" @setScrollingPage="scrolling.page=$event"  @doFilter="searching()" :rowBgColor="rowBgColor" :frmName="frmName">
            <template #[`group`]="{item}">
              {{ name_of_groups(item) }}
            </template>
