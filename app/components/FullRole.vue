<template>
    <div v-if="p_callData" class="w-full flex grow flex-col overflow-auto h-0 z-10" :class="ispop ?'fixed':'relative'">
        ...
        <div class="w-full flex grow flex-col overflow-auto h-0">
          ...
          <TheTableView :thead="fields_thead" :selected="selected" @setSelected="selected = $event" :tbody="roles" :fnCallData="callData" :scrolling="scrolling" @setScrollingPage="scrolling.page=$event"  @doFilter="searching()" :rowBgColor="rowBgColor" :frmName="frmName">
            <!-- <template #[`group`]="{item}">
              {{ name_of_groups(item) }}
            </template>
      ...
      <!-- <ThePopupMini :type="'delete'" :show="delete_box" :data="delete_data" :fnClose="toggleDeleteBox"
      ...
      <!-- <ThePopupMini :type="'delete'" :show="delete_box" :data="delete_data" :fnClose="toggleDeleteBox" :fnConfirm="confirmed_delete" />
...
const enabled_copy = computed(()=>{  
  ...
  && useUtils().checkPermission('platform*role.create');
  return result;
})
...
const enabled_edit = computed(()=>{  
  let result = selected.value > -1 
  && [undefined,0].indexOf(dt_selected.value.deleted) > -1
  && useUtils().checkPermissions(['platform*role.modify']);
  return result;
})
...
//   let result = selected.value > -1
//   && useUtils().checkPermission('platform*role.remove') 
//   && [undefined,0].indexOf(dt_selected.value.deleted) > -1;
//   return result;
// })
