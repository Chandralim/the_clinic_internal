<template>
    <div v-if="p_callData" class="w-full flex grow flex-col overflow-auto h-0 z-10" :class="ispop ?'fixed':'relative'">
        <TheHeaderPopup v-if="ispop" :title="'Form Clinic'" :fn="fnClose" class="w-100 flex align-items-center"
        style="color:white;" />
        <div class="w-full flex grow flex-col overflow-auto h-0">
          ...
          <TheTableView :thead="fields_thead" :selected="selected" @setSelected="selected = $event" :tbody="clinics" :fnCallData="callData" :scrolling="scrolling" @setScrollingPage="scrolling.page=$event"  @doFilter="searching()" :rowBgColor="rowBgColor" :frmName="frmName">
            <!-- <template #[`group`]="{item}">
              {{ name_of_groups(item) }}
            </template> -->
...
const enabled_copy = computed(()=>{  
  ...
  && useUtils().checkPermission('platform*clinic.create');
  return result;
})
...
const enabled_edit = computed(()=>{  
  let result = selected.value > -1 
  && [undefined,0].indexOf(dt_selected.value.deleted) > -1
  && useUtils().checkPermissions(['platform*clinic.modify']);
  return result;
})
...
const enabled_remove = computed(()=>{  
  let result = selected.value > -1
  && useUtils().checkPermission('platform*clinic.remove') 
  && [undefined,0].indexOf(dt_selected.value.deleted) > -1;
  return result;
})
